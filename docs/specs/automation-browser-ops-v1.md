...
| 项目 | 当前状态 | 已做 | 未做 |
| --- | --- | --- | --- |
| 任务模型 | 已接入自动化主线 | `TaskFlow` 已支持 `kind: 'jd-signin'` 与 `signin.site: 'jd'`；`signin_task_runs` 已持久化 latest run 与最近运行历史摘要 | 更完整的运行记录浏览、分页与筛选仍未完成 |
| 执行链路 | 已形成基础闭环 | 已有 `JdSigninProvider`、`SigninTaskService`、`signin:*` IPC 与 `SchedulerService.scheduleRetry()` 分流；执行时优先使用京东会话 API，必要时打开页面补领 | 仍未完成长期稳态验证，异常页面结构仍需继续校准 |
...
> 说明：该专项属于 `automation` 主线内的定制任务能力，不单开顶级菜单。
