...
| Spec | 当前状态 | 说明 |
| --- | --- | --- |
| SPEC-023 命令面板 | 基础版已落地 | 已有 `CommandPalette`、快捷键入口与基础命令流；注册表与最近使用体验仍可继续增强 |
| SPEC-024 KPI 趋势迷你图 | 已落地 | `Sparkline` 已进入工作台能力集 |
| SPEC-025 任务执行时间线 | 已落地 | `TaskTimeline` 已进入工作台能力集 |
| SPEC-026 AI 服务层基础架构 | 基础版已落地 | 已有 `AIService`、`ContextManager`、`ToolRegistry`、`LLMProvider`，并已接入 MCP 工具生态 |
| SPEC-027 AI 聊天面板 UI | 基础版已落地 | 已有 `AIChatPanel`、对话入口与工具调用基础交互；流式体验与更深任务助理能力仍待增强 |
| SPEC-028 系统资源环形仪表盘 | 已落地 | `RingGauge` 已进入工作台能力集 |

> 下文仍保留规格目标与验收项；若某些勾选尚未完全同步，请以本节和 `docs/overview/current-status.md` 为现状基线。
...
| 现状参考 | 当前实际完成度请查看 `docs/overview/current-status.md` |
| 基线规格 | V1.0 基线请查看 `docs/specs/v1.0-baseline.md` |

> 如果某项增强已经部分落地，请同时结合代码和 `docs/overview/current-status.md` 判断其真实完成度。
