...
| 项目 | 说明 |
| --- | --- |
| 文档属性 | 本文档定义 V1.0 基线规格、依赖关系与验收标准 |
| 与现状关系 | Spec 中的勾选状态可作为交付参考，但不应替代当前实现盘点 |
| 现状参考 | 当前仓库实现情况请查看 `docs/current-status.md` |
| 架构参考 | 架构分层与调用链路请查看 `docs/architecture.md` |

> 阅读时请区分“规格目标”和“代码现状”：前者回答“应该交付什么”，后者回答“现在已经做到什么”。
...
**描述**：当前版本将浏览器模块收敛为“浏览器会话控制台”：主进程仍基于 WebContentsView 管理标签与导航，渲染层专注展示会话信息、地址栏和标签状态，而不是直接在页面中伪装原生嵌入视图。
...
- [x] 会话隔离：支持按配置使用持久 Session，并支持创建临时隔离 Session
...
**描述**：插件中心前端界面，展示已安装插件，并支持本地目录安装、卸载确认、启用/禁用等操作。
